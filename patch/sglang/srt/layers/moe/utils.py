...
# ==== [dflash2 backport] draft_model_build_scope (upstream semantics, adapted ====
# to this image's MoeFlags field name speculative_context / no in_speculative_scope) ====
@contextmanager
def draft_model_build_scope():
    """Brackets a draft model's CONSTRUCTION: routes the fusion decision to
    the speculative leaf and restores the target's ACTIVE value on exit.

    Adapted: this image's MoeFlags has `speculative_context` (not
    `in_speculative_scope`) and no `disable_shared_experts_fusion` field.
    """
    moe = get_flags().moe
    original_scope = getattr(moe, "in_speculative_scope", moe.speculative_context)
    try:
        moe.speculative_context = True
        yield
    finally:
        moe.speculative_context = original_scope
